...
@section('content')
    ...
    <body>
        <div class="container mt-5">
            ...
            <form action="{{ route('store.veriants.values') }}" method="POST" enctype="multipart/form-data">
                @csrf
                {{-- <div class="mb-3">
                ...
                <div id="veriants">
                    <div class="veriants-group row mb-3">
                        <div class="col-md-3">
                            <select name="veriants_id[]" id="variant_id" class="form-control variant-select" required>
                                <option value="">Select a Varianst</option>
                                {{-- @foreach ($veriants as $veriant)
                                    <option value="{{ $veriant->id }}">{{ $veriant->variant_name }}</option>
                                @endforeach --}}
                            </select>
                        </div>
                        <div class="col-md-3">
                            <input type="text" class="form-control" name="name[]" placeholder="veriants" required>
                        </div>
                        <div class="col-md-2">
